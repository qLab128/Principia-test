...
class LLMClient:
    ...
    def chat_text(
        ...
        *,
        ...
        try:
            with urllib.request.urlopen(req, timeout=self.settings.request_timeout, context=context) as resp:
    ...
    def _openai_responses_text(
        ...
        *,
        ...
        for _ in range(4):
            try:
                body = self._post_json(resolved["base_url"] + "/responses", resolved["api_key"], payload)
                break
            except RuntimeError as exc:
                last_error = exc
