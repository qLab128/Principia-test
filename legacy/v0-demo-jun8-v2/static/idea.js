...
function linkifyReferences(value) {
  ...
  if (!text) return "";
  const pattern = /\b(?:MI|XI|P|TM|B|BL|W)-[A-Z0-9]+\b/g;
  let cursor = 0;
  let output = "";
...
function renderPrincipleMap(map) {
  ...
  return `
    <div class="principle-canvas" style="height:${height}px">
      ...
      <svg viewBox="0 0 980 ${height}" preserveAspectRatio="none">${edgeHtml}</svg>
      ${renderEdgeInspector()}
      ${nodeHtml}
    </div>
  `;
...
async function init() {
  ...
  document.getElementById("ideaCore").innerHTML = `
    ...
    ${block("User Note", idea.user_note)}
...
function startNodeDrag(event) {
  ...
  dragState = {
    ...
    node: nodeButton,
    canvas,
    offsetX: event.clientX - rect.left - left + canvas.scrollLeft,
    offsetY: event.clientY - rect.top - top + canvas.scrollTop,
    moved: false,
  };
...
function moveNodeDrag(event) {
  ...
  const x = Math.max(20, Math.min(720, event.clientX - rect.left + dragState.canvas.scrollLeft - dragState.offsetX));
  const y = Math.max(50, event.clientY - rect.top + dragState.canvas.scrollTop - dragState.offsetY);
...
document.addEventListener("drop", (event) => {
  ...
  const rect = canvas.getBoundingClientRect();
  node.x = Math.max(20, Math.min(720, Math.round(event.clientX - rect.left + canvas.scrollLeft - 125)));
  node.y = Math.max(50, Math.round(event.clientY - rect.top + canvas.scrollTop - 42));
